...
import ncbisplitter.split_ncbi_sequences_file
import os
import pytest
import runpy
import shutil
...
def test_when_number_of_arguments_equals_two_then_ok():
    ...
    assert ncbisplitter.split_ncbi_sequences_file \
...
def test_when_number_of_arguments_not_equals_two_then_throws_invalid_number_of_arguments_exception():
    ...
    with pytest.raises(InvalidNumberofArgumentsError) as pytest_wrapped_e:
        ncbisplitter.split_ncbi_sequences_file.check_if_is_valid_number_of_arguments(number_of_arguments_provided)
    ...
    assert pytest_wrapped_e.type == InvalidNumberofArgumentsError
    assert str(pytest_wrapped_e.value) == invalid_number_of_arguments_message


def test_when_ncbi_sequences_file_not_exists_then_throws_file_not_found_exception():
    inexistent_sequences_file = Path("inexistent_sequences.fasta")
    with pytest.raises(FileNotFoundError) as pytest_wrapped_e:
        ncbisplitter.split_ncbi_sequences_file.check_if_is_valid_sequences_file(str(inexistent_sequences_file))
...
def test_when_ncbi_sequences_file_is_not_fasta_extension_then_throws_invalid_extension_file_exception():
    ...
    with pytest.raises(InvalidExtensionFileError) as pytest_wrapped_e:
        ncbisplitter.split_ncbi_sequences_file.check_if_is_valid_sequences_file(temporary_sequences_file_str)
    invalid_format_file_message = "Only FASTA Extension Files (.fasta) are Allowed!"
    assert pytest_wrapped_e.type == InvalidExtensionFileError
    assert str(pytest_wrapped_e.value) == invalid_format_file_message
    os.remove(temporary_sequences_file)


def test_when_ncbi_sequences_file_has_not_any_description_line_then_throws_invalid_formatted_fasta_file_exception():
    ...
    temporary_sequences_file_str = str(temporary_sequences_file)
    with pytest.raises(InvalidFormattedFastaFileError) as pytest_wrapped_e:
        ncbisplitter.split_ncbi_sequences_file.check_if_is_valid_sequences_file(temporary_sequences_file_str)
    invalid_formatted_fasta_file_message = "'{0}' Has Not Any Description Line!".format(str(temporary_sequences_file))
    assert pytest_wrapped_e.type == InvalidFormattedFastaFileError
    assert str(pytest_wrapped_e.value) == invalid_formatted_fasta_file_message
    os.remove(temporary_sequences_file)


def test_when_ncbi_sequences_file_has_any_invalid_description_line_then_throws_invalid_formatted_fasta_file_exception():
    temporary_sequences_file = Path("sequences.fasta")
    with open(os.path.join(temporary_sequences_file), mode="w") as sequences_file:
...
def test_when_ncbi_sequences_file_has_no_data_then_throws_invalid_formatted_fasta_file_exception():
    temporary_sequences_file = Path("sequences.fasta")
    with open(os.path.join(temporary_sequences_file), mode="w") as sequences_file:
...
def test_when_ncbi_sequences_file_has_all_valid_lines_then_ok():
    temporary_sequences_file = Path("sequences.fasta")
    with open(os.path.join(temporary_sequences_file), mode="w") as sequences_file:
        sequences_file.write(">ValidDescription1|text1\nAAA\n")
        sequences_file.write(">ValidDescription2 |text2\nCCC\n")
    ...
    temporary_sequences_file_str = str(temporary_sequences_file)
    assert ncbisplitter.split_ncbi_sequences_file.check_if_is_valid_sequences_file(temporary_sequences_file_str) is None
    os.remove(temporary_sequences_file)


def test_when_ncbi_sequences_file_has_no_prefix_path_then_return_empty_prefix_path_string():
    sequences_prefix_path_expected = ""
    temporary_sequences_file = Path("sequences.fasta")
    with open(os.path.join(temporary_sequences_file), mode="w"):
        pass
    temporary_sequences_file_str = str(temporary_sequences_file)
    sequences_prefix_path_returned = ncbisplitter.split_ncbi_sequences_file \
        .get_sequences_prefix_path_from_ncbi_sequences_file(temporary_sequences_file_str)
    assert sequences_prefix_path_returned == sequences_prefix_path_expected
    os.remove(temporary_sequences_file)


def test_when_ncbi_sequences_file_has_prefix_path_then_return_prefix_path_string():
...
def test_when_ncbi_sequences_file_valid_then_return_sequences_name_list():
    ...
    temporary_sequences_file_str = str(temporary_sequences_file)
    sequences_name_list_returned = ncbisplitter.split_ncbi_sequences_file \
        .get_sequences_name_list_from_ncbi_sequences_file(temporary_sequences_file_str)
...
def test_when_ncbi_sequences_file_valid_then_return_sequences_data_list():
    ...
    with open(os.path.join(temporary_sequences_file), mode="w") as sequences_file:
        ...
        sequences_file.write(">Sequence3\nGGG\n")
    temporary_sequences_file_str = str(temporary_sequences_file)
    sequences_data_list_returned = ncbisplitter.split_ncbi_sequences_file \
        .get_sequences_data_list_from_ncbi_sequences_file(temporary_sequences_file_str)
...
def test_when_ncbi_sequences_file_valid_then_split_sequences_and_write_to_disk():
    ...
    sequences_data_list_returned = ncbisplitter.split_ncbi_sequences_file \
        .get_sequences_data_list_from_ncbi_sequences_file(temporary_sequences_file_str)
    ncbisplitter.split_ncbi_sequences_file \
...
def test_when_ncbi_sequences_file_has_no_prefix_path_then_write_sequences_list_file_to_disk():
    ...
    sequences_name_list_returned = ncbisplitter.split_ncbi_sequences_file \
        .get_sequences_name_list_from_ncbi_sequences_file(temporary_sequences_file_str)
    ncbisplitter.split_ncbi_sequences_file \
        .write_sequences_fasta_files_index_list_text_file(sequences_prefix_path_returned, sequences_name_list_returned)
    assert sequences_list_file_expected.exists()
    os.remove(sequences_list_file_expected)
    os.remove(temporary_sequences_file)


def test_when_ncbi_sequences_file_has_prefix_path_then_write_sequences_list_file_to_disk():
...
def test_when_execute_main_function_with_valid_ncbi_sequences_file_then_return_successful_termination_code():
    sequence1_file_expected = Path("Sequence1.fasta")
    sequence2_file_expected = Path("Sequence2.fasta")
    sequence3_file_expected = Path("Sequence3.fasta")
    sequences_list_file_prefix_expected = ""
    ...
    with pytest.raises(SystemExit) as pytest_wrapped_e:
        runpy.run_path("ncbisplitter/split_ncbi_sequences_file.py", run_name="__main__")
