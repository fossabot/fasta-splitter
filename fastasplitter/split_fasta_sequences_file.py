...
def get_sequences_file_extension(fasta_sequences_file: str):
    return fasta_sequences_file.rsplit(".", 1)[1]


def check_if_is_valid_fasta_sequences_file(fasta_sequences_file: str):
    if not os.path.isfile(fasta_sequences_file):
        file_not_found_message = "FASTA Sequences File not Found!"
        raise FileNotFoundError(file_not_found_message)
    if get_sequences_file_extension(fasta_sequences_file) not in ["fa", "faa", "fasta", "ffn", "fna", "frn"]:
    ...
    with open(fasta_sequences_file, mode="r") as sequences_file:
    ...
    if header_lines_count == 0:
        invalid_formatted_fasta_file_message = "'{0}' Has Not Any Description Line!".format(str(fasta_sequences_file))
    ...
    if invalid_header_lines_count != 0:
        invalid_formatted_fasta_file_message = "'{0}' Contains {1} Line(s) With Invalid Description Format!" \
            .format(str(fasta_sequences_file), str(invalid_header_lines_count))
        raise fastasplitter.exceptions.InvalidFormattedFastaFileError(invalid_formatted_fasta_file_message)
    if lines_count < 2:
        invalid_formatted_fasta_file_message = "'{0}' Seems a Empty Fasta File!".format(str(fasta_sequences_file))
        raise fastasplitter.exceptions.InvalidFormattedFastaFileError(invalid_formatted_fasta_file_message)


def get_sequences_prefix_path_from_fasta_sequences_file(fasta_sequences_file: str):
    sequences_prefix_path = ""
    sequences_file_path_split = fasta_sequences_file.rsplit("/", 1)
    if len(sequences_file_path_split) > 1:
        sequences_prefix_path = sequences_file_path_split[0] + "/"
    return sequences_prefix_path


def get_sequences_name_list_from_fasta_sequences_file(fasta_sequences_file: str):
    sequences_start_token = ">"
    sequences_name_list = []
    with open(fasta_sequences_file, mode="r") as fasta_sequences_file:
...
def get_sequences_data_list_from_fasta_sequences_file(fasta_sequences_file: str):
    sequences_start_token = ">"
    sequences_data_list = []
...
def write_sequences_fasta_files_index_list_text_file(sequences_prefix_path: str,
                                                     sequences_file_extension: str,
                                                     sequences_name_list: list):
...
def main(argv: list):
    ...
    # GET FASTA SEQUENCES FILE
    fasta_sequences_file = argv[1]

    # VALIDATE FASTA SEQUENCES FILE
    check_if_is_valid_fasta_sequences_file(fasta_sequences_file)

    # GET FASTA SEQUENCES PREFIX PATH
    sequences_prefix_path = get_sequences_prefix_path_from_fasta_sequences_file(fasta_sequences_file)

    # GET FASTA SEQUENCES FILE EXTENSION
    sequences_file_extension = get_sequences_file_extension(fasta_sequences_file)

    # READ FASTA SEQUENCES FILE AND GET SEQUENCES NAME LIST
    sequences_name_list = get_sequences_name_list_from_fasta_sequences_file(fasta_sequences_file)

    # READ FASTA SEQUENCES FILE AND GET SEQUENCES DATA LIST
    sequences_data_list = get_sequences_data_list_from_fasta_sequences_file(fasta_sequences_file)

    # WRITE SEQUENCES FASTA FILES (SPLITTING ORIGINAL FASTA SEQUENCES FILE INTO INDIVIDUAL SEQUENCES FILES)
    write_sequences_fasta_files_from_sequences_lists(sequences_prefix_path,
                                                     sequences_file_extension,
                                                     sequences_name_list,
                                                     sequences_data_list)

    # WRITE SEQUENCES FASTA FILES INDEX LIST (INDEX OF INDIVIDUAL SEQUENCES FILES)
    write_sequences_fasta_files_index_list_text_file(sequences_prefix_path,
                                                     sequences_file_extension,
                                                     sequences_name_list)
